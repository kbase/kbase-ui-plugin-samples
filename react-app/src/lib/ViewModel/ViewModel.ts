...
import UserProfileClient, {
  UserProfile,
} from "@kbase/ui-lib/lib/lib/comm/coreServices/UserProfile";
...
export type MetadataField = MetadataControlledField | MetadataUserField;
...
export default class ViewModel {
  ...
  constructor({
    ...
    const sampleServiceClientParams: SampleServiceClientParams = {
      url: this.sampleServiceURL,
      timeout,
    };
    if (token !== null) {
      sampleServiceClientParams.token = token;
    }
    this.sampleService = new SampleServiceClient(sampleServiceClientParams);
  }
  ...
  async fetchUsers({
    ...
    if (this.token !== null) {
      userProfileParams.token = this.token;
    }
    const userProfileClient = new UserProfileClient(userProfileParams);
    ...
    return profiles.map((profile) => {
      const {
        ...
        profile: {
          synced: { gravatarHash },
          userdata: { gravatarDefault, avatarOption },
        },
      } = profile;
      return {
        username,
  ...
  async fetchLinkedData({
    ...
    const workspaceCientParams: ServiceClientParams = {
      url: this.workspaceURL,
      timeout: UPSTREAM_TIMEOUT,
    };
    if (this.token !== null) {
      workspaceCientParams.token = this.token;
    }
    const workspaceClient = new Workspace(workspaceCientParams);
    ...
    const result = dataLinks.links.map((dataLink) => {
  ...
  async getSample(params: GetSampleParams): Promise<GetSampleResult> {
    ...
    const { fields } = await this.sampleService.get_field_definitions({
      keys: controlledKeys,
    });
    ...
    const controlledFields: Array<MetadataControlledField> = Object.entries(
      rawRealSample.meta_controlled
    ).map(([key, { value, units }]): MetadataControlledField => {
        ...
        switch (def?.type) {
          case "number":
            if (typeof value !== "number") {
              throw new Error(
                `Field "${
                  def.kbase.sample.key
                }" should be string but is a "${typeof value}"`
              );
            }
            return {
              type: "number",
    ...
    const userFields: Array<MetadataUserField> = Object.entries(
    ...
    const metadata = allFields.map<MetadataField>((templateField) => {
      ...
      if (!fieldDefinition) {
        ...
        throw new Error(`Sorry, field "${templateField.key}" is not defined`);
      }
        ...
        switch (fieldDefinition.type) {
          case "string":
            const y: FieldStringValue = {
              ...
              stringValue:
                fieldValue === null ? fieldValue : (fieldValue.value as string),
            };
          ...
          case "number":
            const x: FieldNumberValue = {
              ...
              // unit: fieldValue !== null ? fieldValue.units : undefined,
              numberValue:
                fieldValue === null ? fieldValue : (fieldValue.value as number),
            };
            if (fieldValue !== null && "units" in fieldValue) {
