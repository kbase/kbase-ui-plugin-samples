import { ParamsSpec, PathSpec, QuerySpec, RouteSpec } from "./RouteSpec";
...
export function transformPathSpec(
  path: string,
): [PathSpec, QuerySpec | undefined] {
  // split on query
  ...
  // create query spec
  const querySpec: QuerySpec = queryPart.split("&").reduce<QuerySpec>(
    (querySpec, queryField) => {
